...
const replaceAlias = input => {
  const index = input.indexOf("alias")
  if(index === -1) return input

  for(let i = index + 8; i < input.length; i++)
    if(input.charAt(i) === "\"") 
      return input.substring(0, index - 1) + input.substring(i + 2, input.length)
}
  ...
  get (command) {
    return new Promise((resolve, reject) => {
      ...
      socket.on('data', (data) => {
        ...
        //Fix the thing that makes json.parse go boom
        data = replaceAlias(data)

        data = JSON.parse(data);

        //Add a custom name instead of the error prone one
        data.alias = "Mr Big Ds plug"
